...
export async function POST(req: Request, { params }: { params: { verificationId: string } }) {
  ...
  await prisma.auditLog.create({ data: { scope: "PLATFORM", orgId: verification.orgId, action: `HQ_ORG_${decision}`, metadata: { reason } } });
  await sendPlatformEmail("admin@internflow.com", `Verification ${decision}`, `${verification.organization.name} was ${decision}. ${reason || ""}`);
