...
export async function POST(req: Request, { params }: { params: { opportunityId: string } }) {
  ...
  const actor = await prisma.user.findUnique({ where: { email: sessionEmail } });
  ...
  if (!opportunity || opportunity.status !== "PUBLISHED") {
    return NextResponse.redirect(new URL("/app/student?error=invalid-opportunity", req.url));
  }

  const activeEnrollment = await prisma.enrollment.findFirst({
    where: { userId: actor.id, status: { in: ACTIVE_ENROLLMENT_STATUSES as unknown as string[] } },
    select: { id: true, organizationId: true }
  });

  if (activeEnrollment && activeEnrollment.organizationId !== opportunity.organizationId) {
    return NextResponse.redirect(new URL("/app/student?error=active-enrollment", req.url));
  }

  const existing = await prisma.application.findFirst({
    where: { userId: actor.id, opportunityId: params.opportunityId, status: { not: "REJECTED" } },
    select: { id: true }
  });
  if (existing) {
    return NextResponse.redirect(new URL("/app/student?notice=already-applied", req.url));
  }
  ...
  await prisma.auditLog.create({ data: { userId: actor.id, action: "APPLICATION_SUBMITTED", metadata: { applicationId: application.id } } });
