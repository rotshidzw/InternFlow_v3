      ...
      <div className="rounded-2xl border border-slate-200 bg-white/80 p-4 shadow-sm backdrop-blur">
        ...
        {meetings.map((m) => (
          <div key={m.id} className="mt-2 rounded border border-slate-200 bg-white p-2 text-sm">
            ...
            <form action={`/api/hq/meetings/${m.id}/remind`} method="post" className="mt-1"><button className="rounded border border-slate-300 bg-white text-slate-900 px-2 py-1 text-xs">Send reminder to tenant contacts</button></form>
          </div>
        ))}
      </div>
    </div>
