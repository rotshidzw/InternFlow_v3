  ...
  return (
    <div className="space-y-4">
      ...
      <form className="grid gap-2 rounded-2xl border border-slate-200 bg-white/80 p-3 shadow-sm backdrop-blur md:grid-cols-5">
        <select name="status" defaultValue={searchParams?.status ?? ""} className="rounded border border-slate-300 px-2 py-2 text-sm"><option value="">Any status</option><option>OPEN</option><option>IN_PROGRESS</option><option>RESOLVED</option></select>
        <select name="priority" defaultValue={searchParams?.priority ?? ""} className="rounded border border-slate-300 px-2 py-2 text-sm"><option value="">Any priority</option><option>LOW</option><option>MEDIUM</option><option>HIGH</option><option>URGENT</option></select>
        <select name="category" defaultValue={searchParams?.category ?? ""} className="rounded border border-slate-300 px-2 py-2 text-sm"><option value="">Any category</option><option>GENERAL</option><option>BILLING</option><option>TECHNICAL</option><option>ONBOARDING</option></select>
        <select name="orgId" defaultValue={searchParams?.orgId ?? ""} className="rounded border border-slate-300 px-2 py-2 text-sm"><option value="">Any tenant</option>{orgs.map((o)=><option key={o.id} value={o.id}>{o.name}</option>)}</select>
      ...
      {tickets.map((t) => (
        <div key={t.id} className="rounded-2xl border border-slate-200 bg-white/80 p-4 shadow-sm backdrop-blur">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <form action={`/api/hq/support/${t.id}/action`} method="post" className="flex flex-wrap gap-2"><input type="hidden" name="action" value="REQUEST_INFO" /><input name="message" placeholder="Ask tenant for details" className="rounded border border-slate-300 px-2 py-1 text-xs" /><button className="rounded border border-slate-300 px-2 py-1 text-xs">Request info</button></form>
