...
export default async function TenantDetailPage({ params }: { params: { tenantId: string } }) {
  ...
  const [verifications, metrics, tickets, meetings, contacts] = await Promise.all([
    ...
    prisma.membership.findMany({ where: { organizationId: tenant.id, role: { in: ["PROVIDER_ADMIN", "COORDINATOR", "SUPERVISOR"] } }, include: { user: true }, take: 10 })
  ]);
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white/80 p-4 shadow-sm backdrop-blur">
          ...
          <p className="mt-2 text-xs text-slate-600">Tenant communication emails: {contacts.map((c) => c.user.email).join(", ") || "None"}</p>
