    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <form action="/api/hq/users" method="post" className="mt-3 grid gap-2 md:grid-cols-3">
          <select name="userId" className="rounded border border-slate-300 px-2 py-2 text-sm">{users.map((u)=><option key={u.id} value={u.id}>{u.email}</option>)}</select>
          <select name="role" className="rounded border border-slate-300 px-2 py-2 text-sm">
