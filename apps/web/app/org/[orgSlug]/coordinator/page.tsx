...
export default async function CoordinatorPage({ params }: { params: { orgSlug: string } }) {
  ...
  const [cohorts, enrollments, pendingDocs, logbooks] = await Promise.all([
    ...
    prisma.document.findMany({ where: { status: { in: ["SUBMITTED", "SCAN_PENDING", "SCAN_FAILED"] }, user: { memberships: { some: { organizationId: access.membership.organizationId } } } }, include: { user: true }, take: 12 }),
    prisma.logbookEntry.findMany({ include: { approvals: true, user: true }, take: 14, orderBy: { createdAt: "desc" } })
  ]);
    ...
    <AppShell orgSlug={params.orgSlug} role={access.membership.role} orgName={access.membership.organization.name}>
      ...
      <section className="mt-4 grid gap-3 md:grid-cols-2">
        <div className="rounded-xl border border-white/15 bg-white/5 p-4">
          ...
          {cohorts.map((c) => <p key={c.id} className="mt-1 text-sm">{c.name} · {c.program.name}</p>)}
      ...
      <section className="mt-3 rounded-xl border border-white/15 bg-white/5 p-4">
        ...
        {enrollments.slice(0,6).map((e)=>(<form key={e.id} action={`/api/enrollments/${e.id}/stipend`} method="post" className="mt-2 flex items-center gap-2 text-sm">
          <span className="min-w-[180px]">{e.user.email}</span>
          <input name="month" defaultValue="2026-02" className="rounded border border-white/20 bg-slate-950/40 px-2 py-1" />
      ...
      <section className="mt-3 rounded-xl border border-white/15 bg-white/5 p-4">
        ...
        <div className="mt-2 flex gap-2">
          <a href={`/api/org/${params.orgSlug}/exports/stipend.csv`} className="rounded-lg border border-white/20 px-3 py-2 text-sm">Stipend register CSV</a>
          <a href={`/api/org/${params.orgSlug}/exports/learners.csv`} className="rounded-lg border border-white/20 px-3 py-2 text-sm">Learner register CSV</a>
        </div>
      </section>
    </AppShell>
  );
